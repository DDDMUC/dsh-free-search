...
// 检查更新的 npm registry 元数据地址（dsh-freesearch 是 npmjs 上的公开包）
const NPM_REGISTRY_URL = "https://registry.npmjs.org/dsh-freesearch/latest";
const PLUGIN_NPM_URL = "https://www.npmjs.com/package/dsh-freesearch";
const PLUGIN_REPO_URL = "https://github.com/DarlingC/dsh-free-search-gemini";
...
function makeBridgeRoutes(settings, search, testEngine) {
  ...
  const handlers = {
    ...
    async updatePlugin() {
      const mode = detectInstallMode();
      if (mode === null) {
        return { ok: false, code: "install-not-found", message: "could not locate dsh-freesearch in any profile" };
      }
      if (mode.isLink) {
        return {
          ok: false,
      ...
      try {
        const result = await new Promise((resolve, reject) => {
          exec("pnpm add dsh-freesearch@latest", { cwd: mode.profileDir, timeout: 120000 }, (error, stdout, stderr) => {
