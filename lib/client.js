  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        keyStorageCredHint: (c) => `保存后写入 ~/.dsh/.credentials.yaml（最高优先级）。当前已配置：${["exa", "tavily", "keenable", "firecrawl", "parallel", "perplexity", "serpbase", "deepseek"].filter((k) => c[k]).map((k) => k.toUpperCase()).join(", ") || "无"}`,
      ...
      en: {
        ...
        keyStorageCredHint: (c) => `Saved to ~/.dsh/.credentials.yaml (highest priority). Currently configured: ${["exa", "tavily", "keenable", "firecrawl", "parallel", "perplexity", "serpbase", "deepseek"].filter((k) => c[k]).map((k) => k.toUpperCase()).join(", ") || "none"}`,
          ...
          if (result.ok) {
            ...
            if (view) {
              ...
              try {
                ...
                if (cred.ok) {
                  const cc = {};
                  const map = { exaApiKey: "exa", tavilyApiKey: "tavily", keenableApiKey: "keenable", firecrawlApiKey: "firecrawl", parallelApiKey: "parallel", perplexityApiKey: "perplexity", deepseekApiKey: "deepseek", serpbaseApiKey: "serpbase" };
...
react_jsx_runtime.jsx("div", {
    ...
    function apply(ctx) {
      ...
      // 挂 0.1.6-alpha.2+ 的插件页 slot：侧栏 插件 → 已安装 → free-search → 行配置页。
